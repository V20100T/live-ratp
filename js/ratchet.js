...
!(function() {
  ...
  var getTarget = function(e) {
    ...
    if (!target ||
      ...
      target.hash && target.href.replace(target.hash, '') === location.href
  ...
  var popstate = function(e) {
    ...
    if (direction === 'back') {
      transitionFrom = JSON.parse(direction === 'back' ? cacheMapping.cacheForwardStack :
        cacheMapping.cacheBackStack);
      transitionFromObj = getCached(transitionFrom[transitionFrom.length -
        1]);
    } else {
    ...
    transition = direction === 'back' ? transitionMap[transitionFromObj.transition] :
    ...
    if (transitionFromObj.transition) {
      activeObj = extendWithDom(activeObj, '.content', activeDom.cloneNode(
        true));
      for (key in bars) {
  ...
  var PUSH = function(options) {
    ...
    options.container = options.container || options.transition ? document.querySelector(
  ...
  var parseXHR = function(xhr, options) {
    ...
    if (/<html/i.test(responseText)) {
      ...
      head.innerHTML = responseText.match(/<head[^>]*>([\s\S.]*)<\/head>/i)[
        0];
      body.innerHTML = responseText.match(/<body[^>]*>([\s\S.]*)<\/body>/i)[
        0];
    } else {
  ...
  //https://github.com/twbs/ratchet/issues/636
...
!(function() {
  ...
  var getTarget = function(target) {
    var i;
    var segmentedControls = document.querySelectorAll(
...
!(function() {
  ...
  var getScroll = function() {
    if ('webkitTransform' in slider.style) {
      var translate3d = slider.style.webkitTransform.match(
        /translate3d\(([^,]*)/);
  ...
  var setSlideNumber = function(offset) {
    ...
    slideNumber = Math[round](getScroll() / (scrollableArea / slider.children
      .length));
    slideNumber += offset;
  ...
  var onTouchMove = function(e) {
    ...
    resistance = slideNumber === 0 && deltaX > 0 ? (pageX / sliderWidth) +
      1.25 :
      slideNumber === lastSlide && deltaX < 0 ? (Math.abs(pageX) /
        sliderWidth) + 1.25 : 1;
  ...
  var onTouchEnd = function(e) {
    ...
    setSlideNumber(
      (+new Date()) - startTime < 1000 && Math.abs(deltaX) > 15 ? (deltaX <
        0 ? -1 : 1) : 0
    );
...
!(function() {
  ...
  window.addEventListener('touchstart', function(e) {
    ...
    var offset = toggle.classList.contains('active') ? (toggleWidth -
      handleWidth) : 0;
  ...
  window.addEventListener('touchmove', function(e) {
    ...
    if (distanceX > offset) {
      return (handle.style.webkitTransform = 'translate3d(' + offset +
        'px,0,0)');
    }

    handle.style.webkitTransform = 'translate3d(' + distanceX + 'px,0,0)';

    toggle.classList[(distanceX > (toggleWidth / 2 - handleWidth / 2)) ?
      'add' : 'remove']('active');
  });
